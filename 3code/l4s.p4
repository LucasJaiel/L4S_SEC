...
control MyIngress(inout headers hdr,
    ...
    apply {
        ...
        if (standard_metadata.instance_type == BMV2_V1MODEL_INSTANCE_TYPE_RECIRC) {
            ...
            if (hdr.ipv4.isValid()) {
                // SEPARAÇÃO PROATIVA: Sempre define a fila com base no bit L4S
                if (hdr.ipv4.l4s == 1) {
                standard_metadata.priority = 7; // L4S -> Alta Prioridade
                } else {
                    standard_metadata.priority = 0; // Clássico -> Baixa Prioridade
                }
            }
            ...
            if (flag == 1){            
                ...
                if (hdr.ipv4.l4s != 1){
                    standard_metadata.priority = (bit<3>)0;  // Fila de baixa prioridade
                ...
                if (hdr.ipv4.l4s == 1){
                    standard_metadata.priority = (bit<3>)7;  // Fila de alta prioridade
                }     
            }
        }
    }
...
control MyEgress(inout headers hdr,
    ...
    register<bit<32>>(2) queue_ewma_classic;  // EWMA da fila 0 por porta ativa
    register<bit<32>>(2) queue_ewma_l4s;      // EWMA da fila 7 por porta ativa
        ...
        if (standard_metadata.instance_type == BMV2_V1MODEL_INSTANCE_TYPE_EGRESS_CLONE) {
            ...
            if (standard_metadata.egress_port == 3) {
                if (standard_metadata.qid == 0) {
                    // Fila 0 (Classic) da porta 3 → índice 0
                    queue_ewma_classic.write(0, EWMA);
                } else if (standard_metadata.qid == 7) {
                    // Fila 7 (L4S) da porta 3 → índice 0
